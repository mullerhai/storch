...
import org.bytedeco.pytorch.MultiMarginLossImpl

final class MultiMarginLoss extends LossFunc {
  override private[torch] val nativeModule: MultiMarginLossImpl = MultiMarginLossImpl()
...
object MultiMarginLoss {
  ...
  def apply() : MultiMarginLoss = new MultiMarginLoss()
}