...
final class FMEmbedding[ParamType <: FloatNN | ComplexNN: Default](
    ...
    val paddingIdx: Option[Int] = None,
    val maxNorm: Option[Float] = None,
    val normType: Option[Float] = Some(2.0f),
    val scaleGradByFreq: Boolean = false,
  ...
  def forward(t: Tensor[ParamType]): Tensor[ParamType] = fromNative(nativeModule.forward(t.native))

  def apply(
      indices: Tensor[Int64 | Int32],
      weight: Option[Tensor[ParamType]] = None
  ): Tensor[ParamType] = indices match {
