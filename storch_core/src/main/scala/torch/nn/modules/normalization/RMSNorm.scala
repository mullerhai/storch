...
final class RMSNorm[ParamType <: FloatNN | ComplexNN: Default](
  ...
  def apply(input: Tensor[ParamType]): Tensor[ParamType] = {
    val output = if elementWiseAffine then torch.rms_norm(input,normalizedShape.map(_.toLong),Some(weight.to(DType.float32)),Some(eps)) else torch.rms_norm(input,normalizedShape.map(_.toLong),None,Some(eps))
