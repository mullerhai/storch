...
import org.bytedeco.pytorch.{GLUOptions, GLUImpl, SELUOptions, SoftmaxOptions}
...
/** Applies the Softmax function to an n-dimensional input Tensor rescaling them so that the
  ...
  */
final class GLU[D <: DType: Default](val dim: Int = -1) extends TensorModule[D]:
...
object GLU {
  def apply[D <: DType: Default](dim: Int = -1): GLU[D] = new GLU(dim)
}
