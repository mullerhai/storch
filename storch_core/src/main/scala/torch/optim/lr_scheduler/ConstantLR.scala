...
//  override var verbose: Boolean = verbose
  require(factor > 0 && factor <= 1.0, "Constant multiplicative factor expected to be between 0 and 1.")
...
//  optimizer.param_groups.
  ...
  override def get_lr(): Seq[Float] = {
    ...
    last_epoch match {
      ...
      case _ =>
        optimizer.param_groups.map(param => param.paramGroup.options().get_lr().toFloat  * (1.0f / factor))
    }
  }
