...
//  val gamma: Float = gamma
  // 计算每个epoch的gamma指数
  private val milestone_counts: Map[Int, Int] = milestones.sorted.groupBy(identity).mapValues(_.length).toMap
  ...
  override def get_lr(): Seq[Float] = {
    ...
    if (!milestone_counts.contains(last_epoch)) {
      ...
      optimizer.param_groups.map(group =>
        group.paramGroupDict("lr").asInstanceOf[Float] * math.pow(gamma, milestone_counts(last_epoch)).toFloat
      )
    }
  }
