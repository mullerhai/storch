...
object DistributedSequentialDataLoader {
  ...
  def apply(dataset: JavaDataset, sampler: DistributedSequentialSampler, option: TorchDataLoaderOptions) =
    new DistributedSequentialDataLoader(dataset, sampler, option.batch_size, option.shuffle, option.num_workers, option.max_jobs, option.drop_last, option.in_order, option.timeout)
}
