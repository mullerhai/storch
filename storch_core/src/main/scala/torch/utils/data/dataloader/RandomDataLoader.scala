...
object RandomDataLoader {
  ...
  def apply(dataset: JavaDataset | DatasetTrait, sampler: RandomSampler, option: TorchDataLoaderOptions) =
    new RandomDataLoader(dataset, sampler, option.batch_size, option.shuffle, option.num_workers, option.max_jobs, option.drop_last, option.in_order, option.timeout)
}

class RandomDataLoader(dataset: JavaDataset | DatasetTrait,
