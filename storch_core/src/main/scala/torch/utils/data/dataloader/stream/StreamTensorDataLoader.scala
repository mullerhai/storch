...
object StreamTensorDataLoader {
  def apply(dataset: StreamTensorDataset, sampler: StreamSampler, option: TorchTensorDataLoaderOptions) =
    new StreamTensorDataLoader(dataset, sampler, option.batch_size, option.shuffle, option.num_workers, option.max_jobs, option.drop_last, option.in_order, option.timeout)
}
