...
object DistributedSequentialTensorDataLoader {
  def apply(dataset: NormalTensorDataset, sampler: DistributedSequentialSampler, option: TorchTensorDataLoaderOptions) =
    new DistributedSequentialTensorDataLoader(dataset, sampler, option.batch_size, option.shuffle, option.num_workers, option.max_jobs, option.drop_last, option.in_order, option.timeout)
}
