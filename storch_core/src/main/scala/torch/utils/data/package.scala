...
//  def poisson_nll_loss[D <: FloatNN | ComplexNN](
//      input: Tensor[D],
//      target: Tensor[D],
...
//  )(implicit cg: ReferenceManager): Tensor[D] = {
...
//  def binary_cross_entropy[D <: FloatNN | ComplexNN](self: Tensor, target: Tensor, weight: Option[Tensor] = None, reduction: Long = internal.Reduction.Mean.swigValue())(implicit rm: ReferenceManager): Tensor = Tensor(swig.binary_cross_entropy(self.underlying, target.underlying, weight.map(_.underlying), reduction))
...
//    self
//  }
//  def gelu[D <: FloatNN | ComplexNN](self: Tensor)(implicit rm: ReferenceManager): Tensor = Tensor(swig.gelu(self.underlying))
