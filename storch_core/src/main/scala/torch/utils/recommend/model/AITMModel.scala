...
final class AITMModel[ParamType <: FloatNN : Default](
  ...
  val bottomLayers = for i <- 0 until (task_num) yield MultiLayerPerceptron(embed_output_dim, bottom_mlp_dims, dropout, false)
  val bottom = ModuleList[ParamType](bottomLayers *)
  val towerLayers = for i <- 0 until (task_num) yield layer.MultiLayerPerceptron(bottom_mlp_dims.last, tower_mlp_dims, dropout)
