...
/**
 * Scala 包装器类，封装 Java 的 ExperimentalConfig 实现
 * 映射所有构造函数、属性方法及操作符方法
 */
class ExperimentalConfigWrapper private (private val underlying: ExperimentalConfig) {
  ...
  def profilerMeasurePerKernel_=(setter: Boolean): Unit = underlying.profiler_measure_per_kernel(setter)
  ...
  def disableExternalCorrelation_=(setter: Boolean): Unit = underlying.disable_external_correlation(setter)
