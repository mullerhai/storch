...
/**
 * Scala 包装器类，封装 Java 的 AutogradState 实现
 * 映射所有静态方法、构造函数及布尔标志的 get/set 方法
 */
class AutogradStateWrapper private (private val underlying: AutogradState) {
  ...
  def setMultithreadingEnabled(enabled: Boolean): Unit = underlying.set_multithreading_enabled(enabled)
