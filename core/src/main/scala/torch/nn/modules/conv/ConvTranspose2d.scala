...
import torch.nn.modules.conv.ConvTranspose2d.PaddingMode
...
final class ConvTranspose2d[ParamType <: FloatNN | ComplexNN: Default](
  ...
  private val options = new ConvTranspose2dOptions(inChannels.toLong, outChannels.toLong, toNative(kernelSize))
  stride match {
  ...
  padding match {
    ...
    case s: Option[Int] =>
      if s.isDefined then options.padding().put(Array(s.get.toLong, s.get.toLong)*)
    case s: Option[(Int, Int)] =>
  ...
  outputPadding match {
    ...
    case s: Option[Int] =>
      if s.isDefined then options.output_padding().put(Array(s.get.toLong, s.get.toLong)*)
    case s: Option[(Int, Int)] =>
      if s.isDefined then options.output_padding().put(Array(s.get._1.toLong, s.get._2.toLong)*)
  }
  dilation match {
  ...
  override private[torch] val nativeModule: ConvTranspose2dImpl = ConvTranspose2dImpl(options)
...
object ConvTranspose2d:
  ...
  ): ConvTranspose2d[ParamType] = new ConvTranspose2d(
    in_channels,
    out_channels,
    kernel_size,
    stride,
